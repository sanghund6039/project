            ...
            init: function(container){
                ...
                if($(container+" .datepicker").length > 0){
                   $(container+" .datepicker").datepicker({
                	   //nextText: "", prevText: "",showOtherMonths: true, selectOtherMonths: true, dateFormat: "yy-mm-dd"
                	    changeYear: true,
	               		yearRange: 'c-50:c+50',
...
function inputDateComparison(obj, strId, endId) {

    // 날짜 입력 엘리먼트 ID는 startDate(시작일), endDate(종료일)로 동일해야 한다.
    var startDate = inputDateSplit(document.getElementById(strId).value);    // 시작일
    var endDate = inputDateSplit(document.getElementById(endId).value);        // 종료일
    ...
    if(startDate == objDate && startDate > endDate) {
        ...
        obj.value = document.getElementById(endId).value;
        obj.focus();
    } else if(endDate == objDate && endDate < startDate) {

        alert("종료일이 시작일보다 이 전 일수는 없습니다.\n다시 선택하여 주시기 바랍니다.");
        obj.value = document.getElementById(strId).value;
        obj.focus();
    } else {
        return false;
    }
